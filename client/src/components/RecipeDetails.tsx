...
import { getMyShoppingList, deleteItem, postItem } from "../Utils/apiDBServiceShoppingList";
...
const RecipeDetails = () => {
    ...
    function filterItems(itemToRemove: ShoppingListItem) {
        return items.filter(item => {
            return item.name !== itemToRemove.name || item.quantity !== itemToRemove.quantity || item.unit !== itemToRemove.unit
        })
    }

    const addHandlerShoppingList = (data: ShoppingListItem) => {
        const newItem = getItemFromData(data);
        (async () => {
            try {
    ...
    function handleClickRemove(data: ShoppingListItem) {
        const itemToRemove = getItemFromData(data);
        (async () => {
            try {
                //Find item from db with same features
                const itemFromDatabase = items.find(item => {
                    return item.name == itemToRemove.name && item.quantity == itemToRemove.quantity && item.unit == itemToRemove.unit
                })
                if (itemFromDatabase && itemFromDatabase.id) {
                    await deleteItem({ id: itemFromDatabase.id });
                    const filteredItems = filterItems(itemFromDatabase)
    ...
    function getItemFromData(data: ShoppingListItem) {
                                        ...
                                        recipe.sections?.map((section: APIRecipeSection) => {
                                            return section.components.map((component, i) =>
                                                <tr key={i}>
                                                    ...
                                                    <td><ItemCheckbox data={{ name: component.ingredient.name, quantity: component.measurements[0].quantity, unit: component.measurements[0].unit.name }} addHandlerShoppingList={addHandlerShoppingList} handleClickRemove={handleClickRemove} />
                                                    </td >
                                                </tr>)
                                        }) :
